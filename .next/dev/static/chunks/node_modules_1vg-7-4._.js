...
/*! @license DOMPurify 3.4.14 | (c) Cure53 and other contributors | Released under the Apache license 2.0 and Mozilla Public License 2.0 | github.com/cure53/DOMPurify/blob/3.4.14/LICENSE */ function _arrayLikeToArray(r, a) {
...
/* HTML-namespace elements whose child text nodes are serialized *literally*
   ...
   serializer's own literal-text list exactly. */ const LITERAL_TEXT_ELEMENT_NAMES = [
    'style',
    'script',
    'xmp',
    'iframe',
...
/* Per-element end-tag matcher. On an HTML reparse the ONLY token that
   ...
   name such as `</styles` is not mistaken for `</style`. */ const LITERAL_TEXT_CLOSE = function() {
    const map = {};
...
function createDOMPurify() {
    ...
    /* Clobber-safe nodeType / nodeName reads through the cached Node.prototype
     ...
     returns early on a null name) intentionally keep their own reads. */ const _readNodeType = function _readNodeType(node) {
   ...
   */ const _removeAttribute = function _removeAttribute(name, element, attr) {
        ...
        try {
         ...
         name-based removal so the "is" handling below still runs. */ try {
                element.removeAttribute(name);
            } catch (_) {}
        }
   ...
   */ const _isUnsafeNode = function _isUnsafeNode(currentNode, tagName) {
       ...
       equally affected. */ if (SAFE_FOR_XML && currentNode.namespaceURI === HTML_NAMESPACE && LITERAL_TEXT_ELEMENTS[tagName] && (_isNode(currentNode.firstElementChild) || typeof currentNode.textContent === 'string' && regExpTest(LITERAL_TEXT_CLOSE[tagName], currentNode.textContent))) {
            return true;
        }
   ...
   */ const _sanitizeDisallowedNode = function _sanitizeDisallowedNode(currentNode, tagName, root) {
        /* Check if we have a custom element to handle */ if (!FORBID_TAGS[tagName] && _isBasicCustomElement(tagName) && _matchesNameCheck(CUSTOM_ELEMENT_HANDLING.tagNameCheck, tagName)) {
            return false;
        }
   ...
   */ const _sanitizeElements = function _sanitizeElements(currentNode, root) {
       ...
       _handleHookDetachedNode for the full rationale). */ if (_handleHookDetachedNode(currentNode, root)) {
            return true;
        }
        ...
        /* Now let's check the element's type and name */ const tagName = transformCaseFunc(_readNodeName(currentNode));
       ...
       above (see _handleHookDetachedNode for the full rationale). */ if (_handleHookDetachedNode(currentNode, root)) {
            return true;
        }
       ...
       slip past the namespace check entirely. */ const nt = _readNodeType(currentNode);
   ...
   */ // eslint-disable-next-line complexity
    const _isValidAttribute = function _isValidAttribute(lcTag, lcName, value) {
       ...
       _isUnsafeNode. */ if (_isPatchLinkageAttribute(lcName, lcTag)) {
            return false;
        }
        ...
        /* Allow valid aria-* attributes, the value is always URI safe */ if (ALLOW_ARIA_ATTR && regExpTest(ARIA_ATTR$1, lcName)) {
            return true;
        }
       ...
       their own discretion. */ if (!nameIsPermitted) {
            return(// Condition a) covers a basically valid custom element tag name whose
        ...
        /* Check value is safe. First, is attr inert? If so, is safe */ if (URI_SAFE_ATTRIBUTES[lcName]) {
            return true;
        }
        /* Check no script, data or unknown possibly unsafe URI
        unless we know URI values are safe for that attribute */ if (regExpTest(IS_ALLOWED_URI$1, stringReplace(value, ATTR_WHITESPACE$1, ''))) {
            return true;
        }
        /* Keep image data URIs alive if src/xlink:href is allowed */ /* Further prevent gadget XSS for dynamically built script tags */ if ((lcName === 'src' || lcName === 'xlink:href' || lcName === 'href') && lcTag !== 'script' && stringIndexOf(value, 'data:') === 0 && DATA_URI_TAGS[lcTag]) {
            return true;
        }
        /* Allow unknown protocols: This provides support for links that
        are handled by protocol handlers which may be unknown ahead of
        time, e.g. fb:, spotify: */ if (ALLOW_UNKNOWN_PROTOCOLS && !regExpTest(IS_SCRIPT_OR_DATA$1, stringReplace(value, ATTR_WHITESPACE$1, ''))) {
            return true;
        }
        /* Only an empty (binary) value remains safe at this point;
       anything else is presumed unsafe, do not add it back */ return !value;
    };
   ...
   */ const _sanitizeShadowDOM2 = function _sanitizeShadowDOM(fragment) {
        ...
        while(shadowNode = shadowIterator.nextNode()){
         ...
         iterator also surfaces. */ if (_readNodeType(shadowNode) === NODE_TYPE.element) {
     ...
     * from the captured bindings just above - so the two cannot diverge. */ if (hooks.uponSanitizeElement.length > 0 || hooks.uponSanitizeAttribute.length > 0) {
       ...
       keep using and whose return value they ignore — unsanitized. REPORT-2. */ const inPlace = IN_PLACE && typeof dirty !== 'string' && _isNode(dirty);
        if (inPlace) {
         ...
         validation entirely. */ const nn = _readNodeName(dirty);
            if (typeof nn === 'string') {
...
//# sourceMappingURL=node_modules_1vg-7-4._.js.map